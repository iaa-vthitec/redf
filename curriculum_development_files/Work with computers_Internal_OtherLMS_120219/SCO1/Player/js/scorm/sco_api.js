...
function setInteraction(sNum,sId,sType,sResponse,sCorrect,sResult,sWeight,nLatency,sDescription,sIdObjective) {
    ...
    // non RTE-3 LMSs may not be able to handle interactions so turn off error warnings
    _bShowCommError = false;
	...
	// turn back on comm error reporting
    _bShowCommError = true;
}
...
function setObjective(sNum,sId,sCompletion,sPercentComplete,sPassFail,sScore,sDescription) {
    ...
    // send the objective as a cross domain message
	var stmt = new ADL.XAPIStatement(ADL.XAPIWrapper.Agent, ADL.verbs.completed);
	stmt.object = new ADL.XAPIStatement.Activity("course:" + player.courseController.course.buildId + ";objective:" + sId + ";", "objective", sDescription);
	stmt.object.definition.type = "http://adlnet.gov/expapi/activities/objective";
	stmt.result = { 'status': sCompletion, 'passFail': sPassFail, 'score': (Math.round(sScore * 100000) / 1000) + "" };
	ADL.XAPIWrapper.postMessage(stmt);

    // turn error warnings back on
    _bShowCommError = true;
}
...
function scormGetValue(name) {
	...
	if (errCode != "0" && errCode != "403" && !_bCommTerminated) {
		/* there is, return an empty string */
	    if (_bShowCommError) courseController.alert("There was an error communicating with the LMS. Getting: " + name + ". Error code:" + errCode);
        return "";
	} else {
		return value;
	}
...
function scormSetValue(name, value) {
	...
	// see if there was an error - non RTE-3 LMSs will gnerate errors so check _bShowCommError to see if we really want errors
	if (result != "true" && !_bCommTerminated) {
	    if (_bShowCommError) courseController.alert("There was an error communicating with the LMS. Setting: " + name + " to " + value);
    }
...
/*
* call the SCORM GetLMSServiceEndpoint function
*/
function GetLMSServiceEndpoint() {
    if (parent && parent.courseDataService)
        return parent.courseDataService.LdServiceBaseUrl; // Azure course

    var API = getAPI();
    if (API == null)
        return "";

    return API.LMSServiceEndpoint && API.LMSServiceEndpoint();
}
